...
namespace IngameDebugConsole
{
    ...
    public static class DebugLogConsole
    {
        ...
        private static readonly Dictionary<string, ConsoleMethodInfo> methods =
            new Dictionary<string, ConsoleMethodInfo>();
        ...
        static DebugLogConsole()
        {
            ...
            var assemblies = new HashSet<Assembly> {Assembly.GetAssembly(typeof(DebugLogConsole))};
            try
            {
        ...
        public static void LogSystemInfo()
        {
            ...
            stringBuilder.Append("Image Effects: ")
                .Append(SystemInfo.supportsImageEffects ? "supported\n" : "not supported\n");
            ...
            stringBuilder.Append("RenderToCubemap: ")
                .Append(SystemInfo.supportsRenderToCubemap ? "supported\n" : "not supported\n");
        ...
        private static void AddCommand(string command, string description, MethodInfo method, object instance = null)
        {
            ...
            if (isMethodValid)
            {
                ...
                methodSignature.Append(method.DeclaringType).Append(".").Append(method.Name).Append("(");
        ...
        public static void ExecuteCommand(string command)
        {
            ...
            var endIndex = IndexOfChar(command, ' ', 0);
            commandArguments.Add(command.Substring(0, endIndex));

            for (var i = endIndex + 1; i < command.Length; i++)
            {
                if (command[i] == ' ')
                    continue;

                var delimiterIndex = IndexOfDelimiter(command[i]);
                if (delimiterIndex >= 0)
                {
            ...
            else
            {
                ...
                var result = methodInfo.method.Invoke(methodInfo.instance, parameters);
                if (methodInfo.method.ReturnType != typeof(void))
                {
                    ...
                    if (result == null || result.Equals(null))
                        ...
                        Debug.Log("Value returned: " + result);
                }
            }
        }
